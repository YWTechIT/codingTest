...
2차원배열안에 달팽이 모양으로 값을 채우는 문제인데, 사실 이 문제를 풀기위해 1460부터 쭉 풀었다. 총 2가지 유형으로 나눠서 풀 수 있다.

⚡️ while문을 사용하여 `row`, `column` 따로따로 반복문을 돌리는 방법
1. n * m의 크기로 값이 0인 2차원 배열 `arr`을 선언한다.
2. `row(행)`, `column(열)`을 각각 0과 -1로 초기화했는데, 이유는 1씩 증가하면서 해당 위치에 `cnt`가 들어가기 때문이다.
3. 그리고 중간에 행과 열의 크기를 -1씩 감소시켰는데, 반복문이 진행될때마다 채워야하는 값의 크기가 줄어들기 때문이다. `(0, 0)`부터 시작하여 `column`에 값을 채우고 난 이후 `row-1` 범위로 값을 채워야하고 다시 `column-1` 범위로 값을 채워야하기 때문이다.
4. 채워야하는 방향이 `➡️, ⬇️`일때는 `step = 1`의 값으로,  `⬅️, ⬆️`일때는 `step = -1`의 값으로 바꿔줘야 해당 방향으로 값을 채워줄 수 있다.

![](https://images.velog.io/images/abcd8637/post/a674eb5e-a955-450f-8597-2d0ced0b32d9/KakaoTalk_Photo_2021-05-03-12-02-09.jpeg)

⚡️ `dx`, `dy`를 사용하여 좌표를 벗어나지 않게 채우는 방법
1. n * m의 크기로 값이 0인 2차원 배열 `arr`을 선언한다.
2. 달팽이배열이 시작하는 좌표를 지정하고, 방향을 지정한다. 
3. R, D, L, U 순서에 맞게 `dx, dy`를 설정한다.
4. 만약, 범위가 벗어나면 벗어난 값을 지우고 방향을 바꾼 후 다시 진행한다.
